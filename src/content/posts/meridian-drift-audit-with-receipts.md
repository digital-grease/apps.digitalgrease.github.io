---
title: "Meridian: drift audit, with receipts"
...
Ollama is enabled by default in `meridian/config.yaml` because it costs nothing and gives every contributor a local baseline. Frontier-model alternation halves the time-resolution on Opus and GPT-5 Preview individually. That's acceptable when drift on the frontier is a months-scale story, not a weeks-scale one.
